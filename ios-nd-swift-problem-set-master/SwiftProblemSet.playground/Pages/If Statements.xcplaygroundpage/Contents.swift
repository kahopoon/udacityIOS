...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if bike && run {
        ...
        if !bike && !run {
            print("\(name) is not ready for a duathlon because bike and run")
        } else if !bike {
            print("\(name) is not ready for a duathlon because bike")
        } else {
            print("\(name) is not ready for a duathlon because run")
        }
    }
}
